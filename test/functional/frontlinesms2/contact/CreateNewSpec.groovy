...
class CreateNewSpec extends grails.plugin.geb.GebSpec {
	def 'button to create new contact exists and goes to NEW CONTACT page'() {
		...
		then:
			assert btnCreateContact.getAttribute('href') == "http://localhost:8080/frontlinesms2/contact/createContact"
	}
	...
	def 'button to create new group exists and goes to NEW GROUP page'() {
		...
		then:
			assert btnCreateGroup.getAttribute('href') == "http://localhost:8080/frontlinesms2/contact/createGroup"
	}
